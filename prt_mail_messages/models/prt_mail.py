...
TREE_TEMPLATE = (
    ...
    "</tr>"
    "<tr>"
    '<td><p id="related-record" style="font-size: x-small;">'
    ...
    def _notify_record_by_email(
        ...
        **kwargs
        ...
        base_mail_values = {
            ...
            # 2 query, check access + read, may be useless, Falsy, when will it be used?
        ...
        for recipients_group_data in recipients_groups_data:
            ...
            # {company, is_discussion, lang, message, model_description,
            # record, record_name, signature, subtype, tracking_values, website_url}
...
class MailMessage(models.Model):
    ...
    partner_allowed_ids = fields.Many2many(
        string="Recipients",
        comodel_name="res.partner",
        compute="_compute_partner_allowed_ids",
    )
    attachment_allowed_ids = fields.Many2many(
        string="Attachments",
        comodel_name="ir.attachment",
        compute="_compute_attachment_allowed_ids",
    )
    ...
    def unlink_rights_check(self):
        ...
        for rec in self:
            ...
            if rec.message_type == "comment":  # Sent
                # Is Author?
    ...
    # Sometimes user has access to record but does not
    #  have access to author or recipients.
    ...
    def _compute_attachment_allowed_ids(self):
        forbidden_records = []
        for rec in self:
    ...
    def _compute_partner_allowed_ids(self):
        forbidden_partners = self.env["res.partner"]
        for rec in self:
    ...
    def _search_messages(self, args, limit=None, order=None):
        ...
        query_str = (
            'SELECT "mail_message".id, "mail_message".model,'
    ...
    def _search(
        ...
        However sometimes this does not work for us because we would
         like to show only messages posted to the records
        user actually has access to

        Following key in context is used:
        - 'check_messages_access': if not set legacy
        'search' is performed

        For the moment we do not show messages posted
         to mail.channel Model (Discussion Channels)
        Finally we check the following:
        After having received ids
         of a classic search, keep only:
        ...
        # id of the last message fetched (in case we need
        # to perform more fetches to fill the limit)
        last_id = False
